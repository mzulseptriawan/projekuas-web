    ...
    <div class="container-fluid">
        <form action="" method="POST" class="user" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <input type="text" name="keterangan" class="form-control form-control-user formlogin" id="" value=
                "<?php
                date_default_timezone_set('Asia/Jakarta');
                $sekarang = strtotime(date('H:i'));
                if($sekarang >=  strtotime('08:00:00')){
                    echo 'Terlambat';
                } else {
                    echo 'Tepat Waktu';
                }
    ...
    if (isset($_POST['btSimpan'])){
        ...
        if(in_array($extention, $allowedExtention) === true){
            if($size < 1044070){
                ...
                $querysimpan = "INSERT INTO `tbabsen` (`id_absen`, `nip`,
                `tgl_absen`, `waktu_absen`, `keterangan`, `foto`, `folder`) VALUES ('".$idAbsen."','".$nip."'
                , '".$tglAbsen."', '".$waktuAbsen."', '".$ketAbsen."', '".$nama."', '../assets/img/absensi/');";
                
                $simpandata = $koneksi -> query($querysimpan);

                if($simpandata){
                    echo "<script>alert('Absensi telah dicatat.');window.location='index.php?page=guru';</script>";
                } else {
                    echo $koneksi -> error;
                }

            } else {
                echo "<script>alert('Ukuran file melebihi batas yang telah ditentukan.');window.location='index.php?page=absensi';</script>";
            }
        } else {

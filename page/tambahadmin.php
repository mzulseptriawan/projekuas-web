...
<!-- Tambah data admin (Khusus Superuser Admin) -->
<div class="container-fluid">
<form action="#" method="post" enctype="multipart/form-data" class="user">
    <br>

    <input type="number" name="level" class="form-control form-control-user" id="" value="2" hidden>
        ...
        // validasi untuk menyimpan data
            if(in_array($extention, $allowedExtention) === true){
                if($size < 1044070){
                    ...
                    if($simpandata){
                        echo "<script>alert('Data telah berhasil disimpan.');window.location='index.php?page=kelolaadmin';</script>";
                    } else {
                        echo $koneksi -> error;
                    }
                } else {
                    echo "<script>alert('Ukuran file melebihi batas yang telah ditentukan.');window.location='index.php?page=tambahadmin';</script>";
                }
            } else {
